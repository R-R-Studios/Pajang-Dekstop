...
class ReportStockInProvider {
  static Future<List<ReportStockInModel>> searchReportStockIn(
      String merchantId, String startDate, String endDate) async {
    List<ReportStockInModel> list = [];
    await DioService.checkConnection(isUseBearer: true, tryAgainMethod: null)
        .then((value) async {
      var dio = DioClient(value);
      var searchReport =
          await dio.searchReportStockIn(merchantId, startDate, endDate);
      if (searchReport.meta.code! < 300) {
        for (var i = 0; i < searchReport.data.length; i++) {
          list.add(ReportStockInModel.fromJson(
              jsonDecode(jsonEncode(searchReport.data[i]))));
        }
      }
    });
