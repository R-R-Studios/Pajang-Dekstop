...
class DialogSaveTransaction extends StatefulWidget {
  ...
  const DialogSaveTransaction(
      this.typeSaveTransaction, this.salesBloc, this.totalTransactionPrice,
      {Key? key})
      : super(key: key);
  ...
  void validateAndSave(BuildContext context) async {
    if (DialogSaveTransaction._formKey.currentState!.validate()) {
      GlobalFunctions.logPrint(
          "total_payment_customer", "$totalPaymentCustomer");
      GlobalFunctions.logPrint(
          "total_transaction_price", "${widget.totalTransactionPrice}");
      GlobalFunctions.logPrint(
          "transaction_from", "${widget.typeSaveTransaction}");
      Navigator.pop(context);
      bool isConnect = await GlobalFunctions.checkConnectivityApp();
      if (isConnect) {
        await widget.salesBloc.requestMerchantTransaction(
            widget.totalTransactionPrice, totalPaymentCustomer);
      } else {
        await widget.salesBloc.addTransactionFailed(
            widget.totalTransactionPrice, totalPaymentCustomer);
      }
