...
class SalesProvider {
  static Future<ResponseStatusTransaction> requestTransaction(Map<String, dynamic> body) async {
    ...
    await DioService.checkConnection(tryAgainMethod: requestTransaction, isUseBearer: true, isLoading: true).then((value) async {
      print('dio value -> $value');
      var dio = DioClient(value);
      var requestTransaction = await dio.requestMerchantTransaction(body);
      GlobalFunctions.logPrint("merchant_transaction", requestTransaction.toJson());
      responseStatusTransaction = ResponseStatusTransaction.fromJson(requestTransaction.data);
      if(requestTransaction.meta.code! < 300){
        responseStatusTransaction.status = true;
        responseStatusTransaction.responseCode = '${requestTransaction.meta.code}';
  ...
  static Future<List<MerchantProductDB>> merchantListProduct(String search, String type) async{
  ...
  static Future getProducts() async {
    ...
    GlobalFunctions.logPrint("salesModel from product", "${productList[1].toJson()}");
    return productList;
  }
  ...
  static Future requestReturnTransaction(Map<String, dynamic> body) async {
    ...
    await DioService.checkConnection(tryAgainMethod: requestReturnTransaction, isUseBearer: true, isLoading: true).then((value) async {
      print('dio value -> $value');
      var dio = DioClient(value);
      var requestTransaction = await dio.requestReturnTransaction(body);
      GlobalFunctions.logPrint("requestReturnTransaction", requestTransaction.toJson());
      if(requestTransaction.meta.code! < 300){
