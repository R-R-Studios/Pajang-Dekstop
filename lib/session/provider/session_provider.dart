  ...
  static Future requestSession(
    ...
    await DioService.checkConnection(
      ...
      if (requestSession.meta.code! < 300) {
        ...
        var decrypt = FireshipCrypt().decrypt(requestSession.data["secret"],
        ...
        ResponseSession session = ResponseSession(
            authToken: listData[0].split(":")[1].toString(),
            token: listData[1].split(":")[1].toString());

        var utilityBox = await FireshipDatabase.openBoxDatabase(
            FireshipUtilityBox.TABEL_NAME);
