...
class ProductProvider {
  static Future createProduct(Map<String, dynamic> body) async {
    ...
    await DioService.checkConnection(
            tryAgainMethod: createProduct, isUseBearer: true)
        .then((value) async {
  ...
  static Future<List<ProductModel>> listProduct() async {
    ...
    for (int i = 0; i < listProduct.length; i++) {
      var check = listProductDB
          .where((element) => element.id == listProduct[i].product?.productId)
          .toList();
      if (check.length < 1) {
    ...
    await DioService.checkConnection(
      ...
      if (list.meta.code! < 300) {
        for (var i = 0; i < list.data.length; i++) {
          ...
          String createdAt =
              await Core.dateConverter(DateTime.tryParse(data.createdAt));
  ...
  static Future<bool> requestCreateMerchantProduct(
      Map<String, dynamic> body) async {
    bool isSuccess = false;
    GlobalFunctions.logPrint("requestCreateMerchantProduct", body.toString());
    await DioService.checkConnection(
            tryAgainMethod: requestCreateMerchantProduct,
            isUseBearer: true,
            isLoading: true)
        .then((value) async {
      ...
      GlobalFunctions.logPrint(
          "requestCreateMerchantProduct", requestTransaction.toJson());
      ...
      GlobalFunctions.logPrint(
          "Status requestCreateMerchantProduct", '$isSuccess');
    });
    return isSuccess;
  }
}
