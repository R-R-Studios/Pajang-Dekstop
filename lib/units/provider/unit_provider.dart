...
class UnitProvider {
  static Future<List<UnitsModel>> getListUnits() async{
    ...
    await DioService.checkConnection(tryAgainMethod: getListUnits, isUseBearer: true).then((value) async {
    ...
    for(int i = 0; i < listUnits.length; i++){
      var check = unitsDB.where((element) => element.id == listUnits[i].id).toList();
      if(check.length < 1){
  ...
  static Future<List<UnitConversionDB>> getUnitConversion() async{
    var box = await Hive.openBox<UnitConversionDB>(FireshipBox.BOX_UNIT_CONVERSION);
    List<UnitConversionDB> listUnits = [];
    await DioService.checkConnection(showMessage: false,tryAgainMethod: getUnitConversion, isUseBearer: true).then((value) async {
      var dio = DioClient(value);
      var list = await dio.getUnitConversion();
      if(list.meta.code! < 300){
        for(int i = 0; i < list.data.length; i++){
          UnitConversionDB unitConversion = UnitConversionDB.fromJson(jsonDecode(jsonEncode(list.data[i])));
